...
class MyTextSanitizerExtension
{
    ...
    {
        $this->ts         = $ts;
        $this->image_path = API_URL . '/images/form';
    }
...
class MyTextSanitizer
{
    ...
    {
        $this->path_basic  = API_ROOT_PATH . '/class/textsanitizer';
        $this->path_plugin = API_ROOT_PATH . '/Frameworks/textsanitizer';
    ...
    public function getSmileys($isAll = true)
    {
        if (count($this->smileys) == 0) {
            /* @var $apiDB APIMySQLDatabase */
            $apiDB = APIDatabaseFactory::getDatabaseConnection();
            if ($getsmiles = $apiDB->query('SELECT * FROM ' . $apiDB->prefix('smiles'))) {
                while ($smiles = $apiDB->fetchArray($getsmiles)) {
                    $this->smileys[] = $smiles;
                }
            }
        }
    ...
    public function smiley($message)
    {
        ...
        foreach ($smileys as $smile) {
            $message = str_replace($smile['code'], '<img class="imgsmile" src="' . API_UPLOAD_URL . '/' . htmlspecialchars($smile['smile_url']) . '" alt="" />', $message);
        }
     ...
     * @return string
     */
    public function &apiCodeDecode(&$text, $allowimage = 1)
    {
        $patterns       = array();
        $replacements   = array();
        $patterns[]     = "/\[siteurl=(['\"]?)([^\"'<>]*)\\1](.*)\[\/siteurl\]/sU";
        $replacements[] = '<a href="' . API_URL . '/\\2" title="">\\3</a>';
    ...
    public function quoteConv($text)
    {
        ...
        $replacement = _QUOTEC . '<div class="apiQuote"><blockquote>\\1</blockquote></div>';
     ...
     * @param  bool|int $xcode  allow apicode?
    ...
    public function &displayTarea($text, $html = 0, $smiley = 1, $xcode = 1, $image = 1, $br = 1)
    {
        ...
        if ($xcode != 0) {
            ...
            if ($image != 0) {
                // image allowed
                $text =& $this->apiCodeDecode($text);
            } else {
                // image not allowed
                $text =& $this->apiCodeDecode($text, 0);
            }
        }
        if ($br != 0) {
    ...
    {
        return '<div class="apiCode">' . $this->executeExtension('syntaxhighlight', str_replace('\\\"', '\"', base64_decode($match[2])), $match[1]) . '</div>';
    }
    ...
    {
        $GLOBALS['apiLogger']->addDeprecated(__CLASS__ . '::' . __FUNCTION__ . ' is deprecated');
        $str = $this->htmlSpecialChars(str_replace('\"', '"', base64_decode($str)));
        $str =& $this->apiCodeDecode($str, $image);
