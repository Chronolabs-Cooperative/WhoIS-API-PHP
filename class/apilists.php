...
if (!defined('API_LISTS_INCLUDED')) {
    ...
    class APILists
    {
        ...
        public static function getAvatarsList($avatar_dir = '')
        {
            $avatars = array();
            if ($avatar_dir != '') {
                $avatars = APILists::getImgListAsArray(API_ROOT_PATH . '/images/avatar/' . $avatar_dir . '/', $avatar_dir . '/');
            } else {
                $avatars = APILists::getImgListAsArray(API_ROOT_PATH . '/images/avatar/');
            }
        ...
        public static function getAllAvatarsList()
        {
            $avatars = array();
            $dirlist = array();
            $dirlist = APILists::getDirListAsArray(API_ROOT_PATH . '/images/avatar/');
            if (count($dirlist) > 0) {
                foreach ($dirlist as $dir) {
                    $avatars[$dir] = &APILists::getImgListAsArray(API_ROOT_PATH . '/images/avatar/' . $dir . '/', $dir . '/');
                }
            } else {
                return false;
            }
        ...
        public static function getSubjectsList($sub_dir = '')
        {
            $subjects = array();
            if ($sub_dir != '') {
                $subjects = APILists::getImgListAsArray(API_ROOT_PATH . '/images/subject/' . $sub_dir, $sub_dir . '/');
            } else {
                $subjects = APILists::getImgListAsArray(API_ROOT_PATH . '/images/subject/');
            }
        ...
        {
            $lang_list = array();
            $lang_list = APILists::getDirListAsArray(API_ROOT_PATH . '/language/');

            return $lang_list;
        }

        /**
         * APILists::getCountryList()
         *
         * @return array
         */
        public static function getCountryList()
        {
            api_loadLanguage('countries');
            $country_list = array(
                '' => '-',
                'AD' => _COUNTRY_AD,
                'AE' => _COUNTRY_AE,
