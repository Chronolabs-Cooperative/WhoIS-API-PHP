...
class APIForm
{
    ...
    public function __construct($title, $name, $action, $method = 'post', $addtoken = false, $summary = '')
    {
        ...
        if ($addtoken != false) {
            $this->addElement(new APIFormHiddenToken());
        }
    }
    /**
     ...
     *
     * @param string|APIFormElement $formElement reference to a {@link APIFormElement}
    ...
    public function addElement($formElement, $required = false)
    {
        ...
        } elseif (is_subclass_of($formElement, 'apiformelement')) {
            $this->_elements[] = &$formElement;
     ...
     *
     * @return APIFormElement[] array of {@link APIFormElement}s
     */
     ...
     *
     * @param  string $name "name" attribute assigned to a {@link APIFormElement}
     * @return object reference to a {@link APIFormElement}, false if not found
     */
     ...
     *
     * @param APIFormElement $formElement reference to a {@link APIFormElement}
     */
    public function setRequired(APIFormElement $formElement)
    {
        $this->_required[] = &$formElement;
    }
    ...
    public function renderValidationJS($withtags = true)
    {
        ...
        $js .= "function apiFormValidate_{$formname}() { var myform = window.document.{$formname}; ";
        $elements =& $this->getElements(true);
     ...
     *
     * @param APITpl $tpl reference to a {@link Smarty} object object
     * @see      Smarty
     */
    public function assign(APITpl $tpl)
    {
        $i        = -1;
        $elements = array();
        ...
        $tpl->assign($this->getName(), array(
            ...
            'extra'      => 'onsubmit="return apiFormValidate_' . $this->getName() . '();"' . $this->getExtra(),
            'javascript' => $js,
            'elements'   => $elements,
